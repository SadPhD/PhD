...
class EPFDM:
    ...
    def __init__(self, data, k, beam_width, search_depth, nb_target, quality_measure, file, loc_factor, min_support, gen_type, gen_factor, skyline):
        self.k = k
    ...
    # We generate files to build plots that show the exceptional model found and deviation created by the absence of the subgroup
                    ...
                    for val in self.domains[var]:
                        ...
                        if(len(list(set(self.pareto_front_global).intersection(current_ext)))/len(self.pareto_front_global) > self.min_support):
                            ...
                            if(duplicate == False):
                                ...
                                if(self.quality_measure ==  "HV"):
                                    qual_pareto_temp = computeQualityHV(self.nadir_point,targets_current_pf)
                                    qual_pareto = 1 - (qual_pareto_temp/self.hv_pareto_front)
                                else:
                                    qual_pareto = computeQuality(self.quality_measure,self.pareto_front_global,current_pf,copy.deepcopy(self.targets_pareto_front),targets_current_pf)
