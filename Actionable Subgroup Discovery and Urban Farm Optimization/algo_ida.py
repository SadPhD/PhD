...
class AlgorithmClosed:

    def __init__(self, data, a, nb_points):
        if(np.size(data,0) >= 100):
            samples = [0]
            lst_samples = random.sample(range(1, np.size(data,0)), nb_points)
            samples.extend(lst_samples)
            data = data[samples, :]
        else:
            data = data[:nb_points+1, :] 
    ...
    def start(self):
        ...
        min_pattern = self.getMinimalPattern()
        extent_min_patt = SortedSet(list(range(self.obj_count)))
        qual = self.computeQuality(extent_min_patt)
        self.top_pattern.update({frozenset(extent_min_patt):qual})
        self.min_threshold = qual
        self.min_extent = frozenset(extent_min_patt)
        self.call_count += 1
        self.process(min_pattern, extent_min_patt, 0, 0)
        # print(self.toStringTopPattern()) # uncommenting this line if running the subgroup discovery by itself to see the results
        return self.getTopPattern()
    ...
    def computeClosedIntervalPattern(self, extent):
    ...
    def computeMean(self, extent):
        array_tar = self.data_tar_og[list(extent)]
        len_ext = len(array_tar)
        return int(np.sum(array_tar)/len_ext)

    
    def computeBest(self, extent):
        array_tar = self.data_tar_og[list(extent)]
        return int(np.max(array_tar))
    ...
    def toStringTopPattern(self):
        top_pat = sorted(self.top_pattern, key=self.top_pattern.__getitem__, reverse = True)
        res = "Pattern: " + self.toStringIntervalPattern(self.computeClosedIntervalPattern(top_pat[0])) + "\n ----- " + self.toStringExtent(top_pat[0]) + "\n ------------ Quality: " + str(self.computeQuality(top_pat[0])) + "\n" + "--------------------- Mean:" + str(self.computeMean(top_pat[0]))
        return res


    def getTopPattern(self):
        top_pat = sorted(self.top_pattern, key=self.top_pattern.__getitem__, reverse = True)
        return self.computeClosedIntervalPattern(top_pat[0]), self.computeMean(top_pat[0]), self.domains, (process_time() - self.t), self.computeBest(top_pat[0])
        ...
        for i in range(attr, self.attr_count):
            ...
            if not(pos == 1 and i == attr):
                ...
                if ip_right[i][0] <= ip_right[i][1]:
                    self.call_count += 1
                    new_extent = self.computeExtent(ip_right, extent, i)
                    new_opt_est = self.computeTightOptimisticEstimate(new_extent)
                    if(new_opt_est > self.min_threshold):
                        new_closed_ip = self.computeClosedIntervalPattern(new_extent)
                        if(self.verifyCanonicity(ip_right, new_closed_ip, i)):
                            ...
                            if(new_qual > self.min_threshold):
                                self.top_pattern.update({frozenset(new_extent):new_qual})
                                del self.top_pattern[self.min_extent]
                                key_min = min(self.top_pattern.keys(), key=(lambda k: self.top_pattern[k]))
                                self.min_threshold = self.top_pattern[key_min]
            ...
            if ip_left[i][0] <= ip_left[i][1]:
                self.call_count += 1
                new_extent = self.computeExtent(ip_left, extent, i)
                new_opt_est = self.computeTightOptimisticEstimate(new_extent)
                if(new_opt_est > self.min_threshold):
                    new_closed_ip = self.computeClosedIntervalPattern(new_extent)
